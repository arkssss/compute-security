from decode import Decode

c11 = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
c10 = "466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
c9 = ""
c8 = "315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
c7 = ""
c6 = ""
c5 = "3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
c4 = ""
c3 = "32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
c2 = "234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
c1 = ""
decoder = Decode(c11, c10)

# print(decoder.StringToASCII(c1))
# print(decoder.StringToASCII(c2))
# print(decoder.get_hex_code("the"))
# print(chr(decoder.TwoBit_To_Hex('')))
# print(decoder.StringToASCII(decoder.Two_string_OXR(decoder.get_hex_code("the"), decoder.get_hex_code("   "))))

res_11_10 = decoder.Two_string_OXR(c11, c10)
res_10_9 = decoder.Two_string_OXR(c10, c9)
res_11_9 = decoder.Two_string_OXR(c11, c9)
res_11_8 = decoder.Two_string_OXR(c11, c8)
res_11_7 = decoder.Two_string_OXR(c11, c7)
res_11_6 = decoder.Two_string_OXR(c11, c6)
res_11_5 = decoder.Two_string_OXR(c11, c5)
res_11_4 = decoder.Two_string_OXR(c11, c4)
res_11_3 = decoder.Two_string_OXR(c11, c3)
res_11_2 = decoder.Two_string_OXR(c11, c2)
res_11_1 = decoder.Two_string_OXR(c11, c1)

# print(decoder.Two_string_OXR("94814d", "140405"))

# print(decoder.StringToASCII(res))
slides_10_11 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_10[:])
# slides_10_9 = decoder.Two_string_OXR(decoder.get_hex_code("the "), res_10_9[2:])
slides_11_9 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_9[:])
slides_11_8 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_8[:])
slides_11_7 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_7[:])
slides_11_6 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_6[:])
slides_11_5 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_5[:])
slides_11_4 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_4[:])
slides_11_3 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_3[:])
slides_11_2 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_2[:])
slides_11_1 = decoder.Two_string_OXR(decoder.get_hex_code("The secret message is: When using a stream cipher, never use the key more than once"), res_11_1[:])

print(decoder.StringToASCII(slides_10_11))
# print(decoder.StringToASCII(slides_10_9))
print(decoder.StringToASCII(slides_11_9))
print(decoder.StringToASCII(slides_11_8))
print(decoder.StringToASCII(slides_11_7))
print(decoder.StringToASCII(slides_11_6))
print(decoder.StringToASCII(slides_11_5))
print(decoder.StringToASCII(slides_11_4))
print(decoder.StringToASCII(slides_11_3))
print(decoder.StringToASCII(slides_11_2))
print(decoder.StringToASCII(slides_11_1))

# 结果
m11 = "The secret message is: When using a stream cipher, never use the key more than once"

# len(m11) == len(c11)/2
print(len(m11))
print(len(c11)/2)

